...
// изображения. Если кэш уже существует, то мы просто отправляем его пользователю.
// Если кэш не существует, то мы генерируем новое изображение и сохраняем его
// в кэш для последующего использования.
// Если топовых матчей нет, то мы отправляем сообщение об этом пользователю.
// Если произошла ошибка при получении топовых матчей, то мы отправляем сообщение об этом пользователю.
// Если произошла ошибка при отправке изображения, то мы отправляем сообщение об этом пользователю
// и возвращаем ошибку для дальнейшей обработки.
